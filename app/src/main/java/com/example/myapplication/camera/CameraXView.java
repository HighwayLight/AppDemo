...
public class CameraXView  extends FrameLayout {
    ...
    // For pinch-to-zoom
    private CameraXView.PinchToZoomGestureDetector mPinchToZoomGestureDetector;
    ...
    @RequiresPermission(Manifest.permission.CAMERA)
    public void bindToLifecycle(@NonNull LifecycleOwner lifecycleOwner, @NonNull ImageAnalysis analysis){
        mCameraModule.setImgAnalysis(analysis);
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            setCaptureMode(
                    CameraXView.CaptureMode.fromId(
        ...
        mPinchToZoomGestureDetector = new CameraXView.PinchToZoomGestureDetector(context);
    }
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        state.putSerializable(EXTRA_SCALE_TYPE, getScaleType());
    ...
    protected void onRestoreInstanceState(@Nullable Parcelable savedState) {
        ...
        if (savedState instanceof Bundle) {
            ...
            setScaleType((PreviewView.ScaleType) state.getSerializable(EXTRA_SCALE_TYPE));
            ...
            setCaptureMode(CameraXView.CaptureMode.fromId(state.getInt(EXTRA_CAPTURE_MODE)));
        } else {
     ...
     * Returns the scale type used to scale the preview.
     *
     * @return The current {@link CameraXView.CaptureMode}.
     */
    @NonNull
     ...
     *
     * @param captureMode The desired {@link CameraXView.CaptureMode}.
     */
    ...
    /**
     * Takes a picture, and calls {@link ImageCapture.OnImageCapturedCallback#onCaptureSuccess(ImageProxy)}
     * once when done.
     *
     ...
     * {@link ImageCapture.OnImageSavedCallback#onImageSaved(ImageCapture.OutputFileResults)} when done.
     *
    ...
    public enum CaptureMode {
        ...
        static CameraXView.CaptureMode fromId(int id) {
            for (CameraXView.CaptureMode f : values()) {
                if (f.mId == id) {
                    return f;
                }
            }
    ...
    private class PinchToZoomGestureDetector extends ScaleGestureDetector
        ...
        PinchToZoomGestureDetector(Context context) {
            this(context, new CameraXView.S());
        }

        PinchToZoomGestureDetector(Context context, CameraXView.S s) {
            super(context, s);
